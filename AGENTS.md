...
│   ├── node-types.ts                 # Node type semantics, schemas and per-type label vocabulary
...
- **Signal** — One typed workmap node that materially helps the user understand or correct the Agent's direction; the goal is separate from the signals.
- **Goal** — The Agent's current reading of what the user ultimately wants from this session; the anchor every other signal is measured against. A corrected goal is a reward, so declare it early even at low confidence. It names the destination, never the route (routes are Decisions). Written through `set_goal` and stable across `restate` declarations. Update it only when the reading of the user's intent changes or deepens — not when the work phase shifts; phase changes surface through active Tasks. `long-term` optionally marks a standing project-level direction.
- **Understanding** — A fact, synthesis, inference, or hypothesis the Agent currently uses. An unlabeled understanding is the Agent's current, unverified reading; `confirmed` marks what evidence has settled and `ruled out` what evidence has killed, so a dead suspicion stays visible instead of disappearing.
...
- **Option** — A considered alternative for a Decision. A tentative answer to an open question is an Understanding, not an Option. An option is only legal beneath its decision: the root schema rejects the type (ADR 0020).
- **Task** — An action the Agent declares it intends or is doing. A finished action is dropped rather than archived; a side effect that still affects later judgment is rewritten as an Understanding. One level of children expresses supporting structure only, never execution tracking (dependencies, progress rollups, or completion archives). Factual questions get no node type: investigate directly, or ask the user in conversation when only they can answer.
...
Labels are type-scoped; the recommended vocabulary lives in `src/node-types.ts` (goal: unlabeled is the current focus, `long-term` optionally marks a standing direction; decision: considering/chosen; understanding: confirmed/ruled out; task: pending/active). The map is capped at 10 nodes including the goal and children, with one level of children. Every write and restore validates the complete state; over-capacity updates are rejected whole (ADR 0017), as is an option at the root (ADR 0020).
