...
- 与 `pi-workmap` 的差异：文章提供协作方法与临时 notes 习惯；`pi-workmap` 试图把持续出现的 Goal、Understanding、Decision 与 Task 做成 live shared state。
...
- 对产品的启发：Goal/Task、Understanding/Decision rationale、hypothesis/Projection 可分别覆盖这些信息层；但 UI 必须控制信息量，避免 transparency 变成噪声。
