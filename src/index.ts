...
export default function workmapExtension(pi: ExtensionAPI): void {
	...
	pi.registerTool({
		...
		promptGuidelines: [
			"You MUST re-declare the complete map via the `workmap` tool before your first action after every user prompt — every non-empty map needs at least one goal.",
			...
			"Use goal for what the user ultimately wants from this session — the destination, never the route; routes are decisions. Update it only when your reading of their intent changes or deepens, not when the work phase shifts; a standing project-level direction may be marked long-term.",
	...
	pi.registerTool({
		...
		label: "Add drift",
		description:
			"Report a mid-task course change to the workmap: call it the moment you change approach or start working around a problem, and the drift is hoisted directly below the goal. Rejected on an empty map or when the map is at its 10-node capacity.",
